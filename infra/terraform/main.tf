...
resource "aws_iam_policy" "etl_github_policy" {
    name = "${var.project_name}-github-policy"
    description = "policy for GitHub Actions ETL"
    policy = jsonencode({
        Version = "2012-10-17"
        Statement = [
            {
...
resource "aws_iam_user_policy_attachment" "etl-github_attach" {
    user = aws_iam_user.etl_user.name
    policy_arn = aws_iam_policy.etl_github_policy.arn
}
